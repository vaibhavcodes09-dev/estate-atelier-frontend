...
import { 
  MapPin, Bed, Bath, Square, Home, CheckCircle2, 
  Phone, Mail, Share2, Heart, Calendar, ChevronRight, User
} from 'lucide-react';
...
const property = {
  ...
  area: "2,100",
  yearBuilt: "2023",
  description: "Experience luxury living in this stunning modern villa located in the heart of Jaswant Nagar. Featuring an open-concept floor plan, high ceilings, and a private backyard oasis with a swimming pool. The gourmet kitchen is equipped with top-of-the-line appliances and custom cabinetry. Perfect for families looking for comfort and elegance.",
  images: [
  ...
  amenities: [
    "Swimming Pool", "24/7 Security", "Modular Kitchen", "Power Backup", 
    "Garden", "Private Parking", "Gymnasium", "Vastu Compliant"
  ],
  agent: {
    name: "Vikram Singh",
    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-20 mt-12 pt-8">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center text-[13px] font-medium text-slate-500 mb-6">
          ...
          <a href="/all-properties" className="hover:text-[#E93946] transition-colors">Properties</a>
          <ChevronRight className="w-4 h-4 mx-2" />
          <span className="text-[#0E2248] truncate">{property.title}</span>
        </nav>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
          ...
          <div className="flex items-center gap-3 md:justify-end">
            ...
            >
              <Heart className={`w-4 h-4 ${isSaved ? 'fill-[#E93946] text-[#E93946]' : ''}`} /> 
              {isSaved ? 'Saved' : 'Save'}
            </button>
          </div>
        </div>
          ...
          <div className="w-full lg:w-2/3 flex flex-col gap-8">
                ...
                {property.images.map((img, index) => (
                  <button 
                    ...
                    className={`relative h-20 sm:h-24 w-28 sm:w-36 shrink-0 rounded-[14px] overflow-hidden border-2 transition-all ${
                      activeImage === img ? 'border-[#E93946]' : 'border-transparent hover:border-[#0E2248]/30'
                    }`}
                  >
            ...
            <div className="bg-white rounded-[24px] p-6 shadow-sm border border-gray-100 grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="flex flex-col p-4 bg-[#F8FAFC] rounded-[16px]">
                ...
                <span className="text-[13px] text-slate-500 font-medium">Bedrooms</span>
                <span className="text-[16px] font-bold text-[#0E2248]">{property.bhk} Rooms</span>
              </div>
              <div className="flex flex-col p-4 bg-[#F8FAFC] rounded-[16px]">
                <Bath className="w-6 h-6 text-[#E93946] mb-2" />
                <span className="text-[13px] text-slate-500 font-medium">Bathrooms</span>
                <span className="text-[16px] font-bold text-[#0E2248]">{property.baths} Baths</span>
              </div>
              <div className="flex flex-col p-4 bg-[#F8FAFC] rounded-[16px]">
                <Square className="w-6 h-6 text-[#E93946] mb-2" />
                <span className="text-[13px] text-slate-500 font-medium">Area</span>
                <span className="text-[16px] font-bold text-[#0E2248]">{property.area} sq.ft</span>
              </div>
              <div className="flex flex-col p-4 bg-[#F8FAFC] rounded-[16px]">
                <Home className="w-6 h-6 text-[#E93946] mb-2" />
                <span className="text-[13px] text-slate-500 font-medium">Built</span>
                <span className="text-[16px] font-bold text-[#0E2248]">{property.yearBuilt}</span>
              </div>
            </div>

            {/* Description Section */}
            <div className="bg-white rounded-[24px] p-8 shadow-sm border border-gray-100">
              <h3 className="text-xl font-bold text-[#0E2248] mb-4">About this Property</h3>
            ...
            {/* Amenities Section */}
            <div className="bg-white rounded-[24px] p-8 shadow-sm border border-gray-100">
              <h3 className="text-xl font-bold text-[#0E2248] mb-6">Features & Amenities</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-4 gap-x-6">
                {property.amenities.map((amenity, index) => (
                  <div key={index} className="flex items-center gap-3">
                    <CheckCircle2 className="w-5 h-5 text-[#E93946]" />
                    <span className="text-[15px] font-medium text-slate-600">{amenity}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="w-full lg:w-1/3">
            <div className="sticky top-28 flex flex-col gap-6">
              ...
              <div className="bg-white rounded-[24px] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-100">
                <p className="text-[14px] text-slate-500 font-bold uppercase tracking-wider mb-1">Asking Price</p>
                <h2 className="text-4xl font-bold text-[#0E2248] mb-6">{property.price}</h2>
              ...
              <div className="bg-[#0E2248] rounded-[24px] p-8 shadow-lg text-white">
                <h3 className="text-[16px] font-bold text-white/80 mb-6 uppercase tracking-wider">Listed By Agent</h3>
                ...
                <div className="flex items-center gap-4 mb-6">
                  ...
                  />
                  <div>
                    <h4 className="text-xl font-bold text-white mb-1">{property.agent.name}</h4>
                    <p className="text-[13px] text-[#E93946] font-medium">{property.agent.role}</p>
                  </div>
                </div>

                <div className="flex flex-col gap-4">
                  <a href={`tel:${property.agent.phone}`} className="flex items-center gap-3 bg-white/10 hover:bg-white/20 p-3 rounded-[12px] transition-colors">
                    <div className="bg-[#E93946] p-2 rounded-lg">
                      <Phone className="w-4 h-4 text-white" />
                    </div>
                    <span className="font-medium text-[15px]">{property.agent.phone}</span>
                  </a>
                  
                  <a href={`mailto:${property.agent.email}`} className="flex items-center gap-3 bg-white/10 hover:bg-white/20 p-3 rounded-[12px] transition-colors">
                    <div className="bg-white/20 p-2 rounded-lg">
                      <Mail className="w-4 h-4 text-white" />
                    </div>
                    <span className="font-medium text-[15px] truncate">{property.agent.email}</span>
                  </a>
                </div>
