    ...
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${headerClass}`}>
      <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="flex items-center justify-between gap-2 sm:gap-4 lg:gap-6 h-12 sm:h-14 w-full">
          ...
          {/* Logo Component (Single brand logo matching user image, transparent background) */}
          <div className="flex items-center shrink-0">
            <Logo className="h-8 sm:h-10 xl:h-11 w-auto" />
          </div>

          {/* Mobile Right: Minimalist Top Header ([ Logo ] ... [ 📍 Location ]) — EXACTLY MATCHING WISHLIST & ACCOUNT PAGE */}
          <div className="lg:hidden shrink-0">
            <button
              onClick={detectLocation}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-slate-200/80 bg-slate-50 text-xs font-semibold text-slate-700 max-w-[145px] truncate"
          ...
          {/* Desktop & Laptop Center Navigation (Adapts smoothly across 1024px, 1280px, 1440px, 1920px) */}
          ...
          {/* Desktop Right Actions (Unclipped Post Property CTA) */}
          <div className="hidden lg:flex items-center gap-2 xl:gap-3 shrink-0">
            ...
            {/* Standout Primary CTA — GUARANTEED UNCLIPPED WITH shrink-0 flex-shrink-0 */}
            <Link
              to="/add-property"
